...
20 份 FINDINGS 里有若干条已被后来的实验推翻或降级。**只读某一份会踩到作废的结论**，
所以先读这份，再按需要跳转。

最后更新：D 类真模型验收之后。测试 `./thymus/run-tests.sh` 应为 11 files / 151 passed。
...
- 花钱的脚本：`a2-ab` / `a2-feedback` / `a2-jspace` / `check-llm-optin` / `check-intact-damage` /
  `check-d-fallback`（`THYMUS_ARM=full|narrow|split|split-narrow` 选臂）
