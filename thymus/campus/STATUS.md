...
21 份 FINDINGS 里有若干条已被后来的实验推翻或降级。**只读某一份会踩到作废的结论**，
所以先读这份，再按需要跳转。

最后更新：SPEC.md 整份编译成声明跑完之后。测试 `./thymus/run-tests.sh` 应为 11 files / 151 passed。
...
- 「同一份 SPEC 交给 agent 自己写声明」没做——那是本轮原命题的另一半。
  发现 21 里那 6 条声明是人写的（我写的），不是客户填的也不是模型填的。
...
- 花钱的脚本：`a2-ab` / `a2-feedback` / `a2-jspace` / `check-llm-optin` / `check-intact-damage` /
  ...
  `check-spec-full`（SPEC.md 整份声明验收）
