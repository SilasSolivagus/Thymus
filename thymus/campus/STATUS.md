...
25 份 FINDINGS 里有若干条已被后来的实验推翻或降级。**只读某一份会踩到作废的结论**，
所以先读这份，再按需要跳转。

最后更新：propose_tool 配额去重回收之后。测试 `./thymus/run-tests.sh` 应为 13 files / 173 passed。
...
18. **evals 是检测器，检测不到没写进去的东西。** 漏掉的条款不会有用例，报告因此全绿——
    ...
    只是怎么改只有人知道。
19. **冻结前先过机械闸**（`checkSpecHygiene`，纯静态不花钱）。人不可替代的四件事里，
...
- 测试：`./thymus/run-tests.sh`（13 files / 173 passed）
