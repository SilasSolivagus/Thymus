...
23 份 FINDINGS 里有若干条已被后来的实验推翻或降级。**只读某一份会踩到作废的结论**，
所以先读这份，再按需要跳转。

最后更新：探完工具注册边界之后。测试 `./thymus/run-tests.sh` 应为 11 files / 151 passed。
...
6. **调外部 API 前先把它的签名打出来看一眼。** 发现 23 里连着两次把「我调错了」
   读成「被安全机制拦住了」——两者的屏幕输出一模一样。让被测代码把手上有什么、
   拿到了什么打出来，一次就分清。
7. **读会话事件的形状不要凭印象写，先 dump 一条真的出来。** 发现 18 里判定和验收
...
- `propose_tool` 没验真 agent 端到端：模型会不会写出合规提案、好不好用，没测（23）。
