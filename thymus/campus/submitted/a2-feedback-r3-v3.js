...
// 宽泛信号：只在语义模型不可用/无法解析时作兜底，绝不用它来“挡住”模型调用
function looksSuspicious(text) {
  if (/(?:不|没|无|未|别|莫|甭|勿|拒|否)/.test(text)) return true;
  if (/(?:怪|赖|怨|甩|推|责任|埋怨|怪罪)/.test(text)) return true;
  if (/(?:系统|服务器|平台|网站|崩|挂|坏|bug|Bug|故障|异常|宕机)/.test(text)) return true;
  if (/(?:爱.{0,6}(?:不|咋|怎么)|随|算|拉倒|看着办|爱信不信)/.test(text)) return true;
  if (/(?:听不懂|不明白|理解|清楚|懂吗|明白吗)/.test(text)) return true;
...
// 三态：true=有问题，false=合规，null=模型不可用/输出无法解析
...
async function checkAndFix(ctx, text) {
  ...
  // ── 语义判定必须被问到 ─────────────────────────────────────────
  // 词表/正则只是“已知违规”的加速器：命中 → 直接走改写（改写模型同样参与）。
  // 词表没命中的文本，绝不能静默放行——同义改写往往就在这一层漏掉，
  // 因此未命中时【一定】调用语义判定模型，由模型决定是否合规。
  let hasIssue = hits.length > 0;
  if (!hasIssue) {
    const verdict = await judge(ctx, text);   // 模型必须被问到
    if (verdict === false) return text;       // 模型判定合规 → 原样透传
    if (verdict === true) {
      hasIssue = true;
    } else {
      // 模型不可用/无法解析：用宽泛信号兜底，避免违规话术静默放行
