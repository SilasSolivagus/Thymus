/*
 * A2 服务禁语约束插件（v3）
 * - 拦截 llm/stream。先做词表/正则快速预筛；预筛未命中时，仍调用语义审查 LLM 判断，
 *   确保词表覆盖不到的消极/质问/推诿表达也能被拦住（无精确边界 → 必须让模型判定）。
 * - 命中后调用内部 LLM 改写；改写必须保留账期、金额等事实信息，失败则本地替换兜底。
 * - 合规话术（含语义审查误报但模型认为无需改写的情形）原样透传，不改动任何 chunk。
...
const FORBIDDEN_EXACT = [
  ...
  '听明白没有', '听懂了没有', '明白没有', '懂了没有', '听清楚没有', '看清楚没有',
  ...
  '我没义务', '没有义务', '我没这义务', '你自己负责',
  '我也没办法', '我也没有办法', '我们也没办法',
  '怪谁', '怨谁', '赖谁', '怪得了谁', '能怪谁',
  '别怪我', '别怨我', '别赖我', '不怪我', '不怨我', '不赖我',
  '这不怪我', '这不能怪我', '这不怨我', '这不赖我',
  '活该', '你自找的', '这锅我不背', '你爱找谁找谁', '你有病吧', '你傻吗', '你聋了吗', '你瞎了吗'
];
...
const FORBIDDEN_REGEX = [
  ...
  /[你您]\s*听\s*不\s*(懂|明白)/,
  /听\s*不\s*(懂|明白)\s*(吗|么)/,
  /(?:听|看|想|说|做|弄|搞)?\s*(?:明白|清楚|懂)\s*(?:了\s*)?没/,
  /你\s*(?:明白|懂|知道)\s*(?:吗|么)/,
  /你\s*到\s*底\s*(?:听|看|想|懂|明白|说|要|问|闹)/,
  /你\s*是\s*不\s*是\s*(?:听\s*不\s*(?:懂|明白)|傻|瞎|聋)/,
  /你\s*(?:聋|瞎)\s*(?:了\s*)?(?:吗|啊)/,
  /你\s*脑\s*子\s*(?:有\s*问\s*题|进\s*水|不\s*好)/,
  /你\s*有\s*(?:病|毛\s*病)\s*(?:吗|吧)/,
  /你\s*傻\s*(?:吗|呀|啊)/,
  /你\s*什\s*么\s*意\s*思/,
  /你\s*闹\s*够\s*(?:了\s*)?没\s*有/,
  /你\s*听\s*不\s*懂\s*人\s*话/,
  /你\s*听\s*不\s*见\s*(?:吗|啊)/,
  /要\s*我\s*说\s*几\s*遍/,
  /说\s*(?:了|过)\s*多\s*少\s*遍/,
  /讲\s*了\s*多\s*少\s*遍/,
  ...
  /怨\s*(我|我们)/,
  /赖\s*(我|我们)/,
  /谁\s*让\s*你\s*(不|没|这么|这样)/,
  /是\s*你\s*自\s*己\s*(没|不|没有)/,
  /你\s*自\s*己\s*(没|没有|不\s*(交|办|会|懂|弄|设置|提供|配合|注意|确认|检查))/,
  /这?\s*锅\s*我\s*不\s*背/
];

const LOCAL_REPLACEMENTS = [
  ['你到底听明白没有', '我可能没有表达清楚，我再为您说明一下'],
  ['你到底听懂了没有', '我可能没有表达清楚，我再为您说明一下'],
  ['你到底想怎样', '请问还有什么可以帮您'],
  ['你到底想干什么', '请问还有什么可以帮您'],
  ...
  ['不是我的错', '非常抱歉，我会帮您核实处理'],
  ['不是我的事', '非常抱歉，我会帮您核实处理'],
  ['听明白没有', '我可能没有表达清楚，我再为您说明一下'],
  ['听懂了没有', '我可能没有表达清楚，我再为您说明一下'],
  ['听清楚没有', '我可能没有表达清楚，我再为您说明一下'],
  ['看清楚没有', '我可能没有表达清楚，我再为您说明一下'],
  ['明白没有', '我可能没有表达清楚，我再为您说明一下'],
  ['懂了没有', '我可能没有表达清楚，我再为您说明一下'],
  ['这不该我管', '我帮您转达给相关负责人处理'],
  ['不该我管', '我帮您转达给相关负责人处理'],
  ['不归我管', '我帮您转达给相关负责人处理'],
  ['不归我们管', '我帮您转达给相关负责人处理'],
  ...
  ['没辙', '目前暂时无法处理'],
  ['没法子', '目前暂时无法处理'],
  ['怪得了谁', '非常抱歉给您带来不便，我们会协助您核实处理'],
  ['能怪谁', '非常抱歉给您带来不便，我们会协助您核实处理'],
  ['怪谁', '非常抱歉给您带来不便，我们会协助您核实处理'],
  ['怨谁', '非常抱歉给您带来不便，我们会协助您核实处理'],
  ['赖谁', '非常抱歉给您带来不便，我们会协助您核实处理'],
  ['这不怪我', '非常抱歉，我们会尽力协助您处理'],
  ['这不能怪我', '非常抱歉，我们会尽力协助您处理'],
  ['这不怨我', '非常抱歉，我们会尽力协助您处理'],
  ['这不赖我', '非常抱歉，我们会尽力协助您处理'],
  ['别怪我', '非常抱歉，我们会尽力协助您处理'],
  ['别怨我', '非常抱歉，我们会尽力协助您处理'],
  ['别赖我', '非常抱歉，我们会尽力协助您处理'],
  ['不怪我', '非常抱歉，我们会尽力协助您处理'],
  ['不怨我', '非常抱歉，我们会尽力协助您处理'],
  ['不赖我', '非常抱歉，我们会尽力协助您处理'],
  ['这锅我不背', '非常抱歉，我们会尽力协助您处理'],
  ['你爱找谁找谁', '您看这样可以吗'],
  ['你有病吧', '非常抱歉给您带来不好的体验'],
  ['你傻吗', '非常抱歉给您带来不好的体验'],
  ['你聋了吗', '非常抱歉给您带来不好的体验'],
  ['你瞎了吗', '非常抱歉给您带来不好的体验'],
  ['活该', '非常抱歉给您带来不好的体验，我们会协助您处理'],
  ['你自找的', '非常抱歉给您带来不便，我们会协助您处理'],
  ['你到底', '']
];
...
function containsForbidden(text) {
  ...
  if ((text.includes('怪你') || text.includes('怪用户')) && !text.includes('不怪你') && !text.includes('别怪你') && !text.includes('不怪用户') && !text.includes('别怪用户')) return true;
  if ((text.includes('怨你') || text.includes('怨用户')) && !text.includes('不怨你') && !text.includes('别怨你') && !text.includes('不怨用户') && !text.includes('别怨用户')) return true;
  if ((text.includes('赖你') || text.includes('赖用户')) && !text.includes('不赖你') && !text.includes('别赖你') && !text.includes('不赖用户') && !text.includes('别赖用户')) return true;
  if (text.includes('怪你自己') && !text.includes('别怪你自己') && !text.includes('不怪你自己')) return true;
  if (text.includes('你自己的问题') && !text.includes('不是你自己的问题')) return true;
  return false;
}
...
function localRewrite(text) {
  ...
  out = out.replace(/(?:听|看|想|说|做|弄|搞)?\s*(?:明白|清楚|懂)\s*(?:了\s*)?没/g, '我可能没有表达清楚，我再为您说明一下');
  out = out.replace(/你\s*(?:明白|懂|知道)\s*(?:吗|么)/g, '我可能没有表达清楚，我再为您说明一下');
  out = out.replace(/你\s*是\s*不\s*是\s*(?:听\s*不\s*(?:懂|明白)|傻|瞎|聋)/g, '我可能没有表达清楚，我再为您说明一下');
  out = out.replace(/你\s*(?:聋|瞎)\s*(?:了\s*)?(?:吗|啊)/g, '非常抱歉给您带来不好的体验');
  out = out.replace(/你\s*脑\s*子\s*(?:有\s*问\s*题|进\s*水|不\s*好)/g, '非常抱歉给您带来不好的体验');
  out = out.replace(/你\s*有\s*(?:病|毛\s*病)\s*(?:吗|吧)/g, '非常抱歉给您带来不好的体验');
  out = out.replace(/你\s*傻\s*(?:吗|呀|啊)/g, '非常抱歉给您带来不好的体验');
  out = out.replace(/你\s*什\s*么\s*意\s*思/g, '非常抱歉，我重新为您说明一下');
  out = out.replace(/你\s*闹\s*够\s*(?:了\s*)?没\s*有/g, '请问还有什么可以帮您');
  out = out.replace(/你\s*听\s*不\s*懂\s*人\s*话/g, '我可能没有表达清楚，我再为您说明一下');
  out = out.replace(/你\s*听\s*不\s*见\s*(?:吗|啊)/g, '我可能没有表达清楚，我再为您说明一下');
  out = out.replace(/要\s*我\s*说\s*几\s*遍/g, '非常抱歉给您带来困扰，我再为您说明一下');
  out = out.replace(/说\s*(?:了|过)\s*多\s*少\s*遍/g, '非常抱歉给您带来困扰，我再为您说明一下');
  out = out.replace(/讲\s*了\s*多\s*少\s*遍/g, '非常抱歉给您带来困扰，我再为您说明一下');
  ...
  out = out.replace(/怪\s*(?:我|我们)/g, '非常抱歉，我们会尽力协助您处理');
  out = out.replace(/怨\s*(?:我|我们)/g, '非常抱歉，我们会尽力协助您处理');
  out = out.replace(/赖\s*(?:我|我们)/g, '非常抱歉，我们会尽力协助您处理');
  out = out.replace(/谁\s*让\s*你\s*(?:不|没|这么|这样)/g, '非常抱歉，我们会协助您核实处理');
  out = out.replace(/(?:当初|本来)?\s*是\s*你\s*自\s*己\s*(?:没|不|没有)[^。？！，；]*/g, '非常抱歉，我们会协助您核实处理');
  out = out.replace(/你\s*自\s*己\s*(?:没|没有|不\s*(?:交|办|会|懂|弄|设置|提供|配合|注意|确认|检查))[^。？！，；]*/g, '非常抱歉，我们会协助您核实处理');
  out = out.replace(/这?\s*锅\s*我\s*不\s*背/g, '非常抱歉，我们会尽力协助您处理');
  out = out.replace(/你\s*爱\s*找\s*谁\s*找\s*谁/g, '您看这样可以吗');
  if (out.includes('怪你') && !out.includes('不怪你') && !out.includes('别怪你')) out = out.split('怪你').join('非常抱歉给您带来不便');
  if (out.includes('怨你') && !out.includes('不怨你') && !out.includes('别怨你')) out = out.split('怨你').join('非常抱歉给您带来不便');
  if (out.includes('赖你') && !out.includes('不赖你') && !out.includes('别赖你')) out = out.split('赖你').join('非常抱歉给您带来不便');
  return out;
}
...
const REWRITE_PROMPT = `下面是一段校园网客服 agent 对用户说的话。其中可能包含服务禁语（态度消极、强势质问、甩锅推诿类表达），例如：「不可能」「做不到」「没办法」「这不是我的责任」「系统崩了」「系统出bug了」「你听不懂吗」「你听明白没有」「怪谁」等。
...
const JUDGE_SYSTEM = '你是一名客服话术合规审查员。你的职责是判断一段客服回复是否包含服务禁语。';

const JUDGE_PROMPT = `请判断下面这段校园网客服 agent 对用户说的话，是否包含服务禁语：态度消极、强势质问、甩锅推诿类表达。例如「不可能」「做不到」「没办法」「这不是我的责任」「系统崩了」「系统出bug了」「你听不懂吗」「你听明白没有」「怪谁」等，以及其他任何语气消极、质问、推诿、甩锅的说法。

只回答 YES（包含）或 NO（不包含），不要输出任何其他内容。
...
async function* processStream(ctx, options, next) {
  ...
  const vocabHit = containsForbidden(fullText);
  let needsRewrite = vocabHit;

  // 词表/正则未命中时，仍交给语义审查 LLM 判定，避免词表覆盖不到的说法漏网。
  if (!needsRewrite) {
    let verdict = '';
    try {
      verdict = await callLlmText(ctx, JUDGE_SYSTEM, JUDGE_PROMPT.replace('@@TEXT@@', fullText), options, 10000);
    } catch (e) {
      verdict = '';
    }
    const v = String(verdict || '').trim().replace(/^["'「」]+|["'「」]+$/g, '').trim();
    needsRewrite = /^\s*(YES|是)/i.test(v);
  }
  ...
  let finalText = null;
  if (vocabHit) {
    // 词表已命中：改写必须真正改变原文且干净、保留事实，否则用本地替换兜底。
    try {
    ...
    if (!finalText) finalText = localRewrite(fullText);
  } else {
    // 语义判定命中：模型若认为无需改写会原样输出，此时保持原文透传。
    try {
      const rewritten = cleanModelOutput(
        await callLlmText(ctx, REWRITE_SYSTEM, REWRITE_PROMPT.replace('@@TEXT@@', fullText), options, 20000)
      );
      if (rewritten && !containsForbidden(rewritten) && factsPreserved(fullText, rewritten)) {
        finalText = rewritten;
      }
    } catch (e) {
      finalText = null;
  ...
  if (containsForbidden(finalText)) finalText = sanitize(finalText);
  if (containsForbidden(finalText)) finalText = sanitize(finalText);
  ...
  if (!finalText.trim()) finalText = '非常抱歉给您带来困扰，我会尽力协助您处理。';

  // 文本没有实质变化：原样透传原始 chunk，绝不破坏合规话术。
  if (finalText === fullText || normalize(finalText) === normalize(fullText)) {
    yield* chunks;
    return;
  }
