...
3. **跑考题**（`checkSpecEvals`）——必拦 / 必放 / 留出三组数字，哪组掉了对应哪种毛病。
   ...
   （它发出前不再过闸，见发现 29）。**不过就抛错，不冻结。**
   分两级（发现 31 四）：**终点串**（网关兜底串，要带 `terminal: true`）撞上别的约束是
   error，拦住冻结——它退无可退；**非终点**的撞上是提示，运行时会把它换成终点串
   （发现 31，端到端 4/20 → 1/20），后果是话术降级，但那次再裁决只判一次、有方差漏网。
   campus 这份 SPEC 现在过闸：终点串干净（发现 29 八改的），B2 那条是提示——
   它撞 D 是措辞解决不了的（4 条候选全被拦），撞自己那处更像判定器误判。
4. **上线之后每天回流**（`badcase.ts`）——线上样本人标、确定性分流、留出侧只给数字、
   与上一版比。

运行时那侧：工具通道挂调度器（拒绝 + 产出抹除），说话通道挂 `prepareCall`
（缓冲全流、装配后裁决、只换话不停轮；**替代话术再过一遍闸**，退到网关兜底串为止，
开药方的那条不参与——发现 31），另有 `propose_tool` 让 agent 自己长工具
...
| 30 | 换一组同义表达重打分：发现 07 的核心结论稳，但留出集分数只在同组内可比 | `FINDINGS-30-heldout-dependence.md` |
| 31 | 替代话术在运行时再过一遍闸：4/20 → 1/20；跳过开药方的那条是实测逼出来的 | `FINDINGS-31-runtime-readjudication.md` |
...
2. ~~发现 05／06 留下的直接后续~~ **三项都做完了**：
   - 重复 n 次——发现 07。结论被推翻一半，且暴露出第三种失败形态（回归）。
   - 换一组留出集题目重跑——**发现 30**：核心结论稳（还是 r3 那两版泛化），
     但 r1-v3 的 5/6 掉到 2/6，发现 07 里 r1 那条「提升」不泛化。
     留下一条规矩：留出集分数只在同一组题目内部可比，跨组比必须带同轮对照。
   - 多插件——发现 13（比污染严重，是无限递归）。
   - fail-open 打失败——发现 15（两侧兜底都是假的）。
