...
| 11 | J-Space 实测负收益，不引入 | `FINDINGS-11-jspace-eval.md` |
...
0. **先查基线不一致**。发现 11 的三次基线首版全部没加门控、留出集2 全 6/6，
   而发现 05/07 记的是「首版基本都会加门控（5 次里 4 次）」、留出集2 只有 2–3/6。
   同任务同评测同模型。是提示词细微差异，还是方差大到那 4–5 个样本撑不住结论？
   **这条排最前**——如果是方差，发现 05 和 07 的核心表述都要降级。
