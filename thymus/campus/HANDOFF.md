...
### J-Space Cognition Suite V3.6 —— 正在实测，前两次结果为负

`Tiger3807861189/J-Space-Cognition-Suite-V3.6`。推理时认知脚手架（提示词/skill 层），
与 Thymus（运行时/插件/治理层）不同层，不冲突不互替。潜在接触点是「能不能提升
DeepSeek 写插件的正确率」。

实测脚本 `a2-jspace.ts`：A2 写插件任务，冻结评测不动，唯一变量是提示词前面加不加
它的 SKILL.md 全文；每臂 3 次，只测首版不做反馈。偏差写在脚本头部——只喂了 16KB
入口，9 个模块按需加载没实现，所以测的是它能力的下限。

截至目前（日志 `run-log-a2-jspace.txt`，以文件为准）：

| 跑次 | 公开集 | 留出集 | 留出集2 | 未问模型 | dev 轮 | 输出 tok |
|---|---|---|---|---|---|---|
| base/r1 | 9/9 | 6/6 | 6/6 | 0 | 362s | 101660 |
| base/r2 | 9/9 | 5/6 | 6/6 | 0 | 409s | 116860 |
| base/r3 | 9/9 | 6/6 | 6/6 | 0 | 326s | 91974 |
| jspace/r1 | 9/9 | 2/6 | 1/6 | 6 | 507s | 148152 |
| jspace/r2 | 9/9 | 3/6 | 2/6 | 3 | 761s | 211380 |

方向与它自报的 2.53× 提速、2.21× token 下降**相反**：更慢、更贵，且留出集大幅下降、
门控回潮。还差 jspace/r3。结论待跑完再下，但目前没有引入的理由。

**顺带暴露一个要单独查的问题**：这三次基线首版全部没加门控、留出集2 接近满分，
与发现 05/07 记的「首版基本都会加门控（4/5）」差很远。是提示词有细微差异，
还是纯运行方差？没查。这条会影响发现 05/07 的结论强度。
