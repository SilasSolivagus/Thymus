...
声明冻结前先跑 `checkSpecHygiene(bundle)`——纯静态、不调模型，凡是能证明写错的一律拦住：
用例缺组、留出用例是伪装的（逐类型各有检法）、条款文本里把有条件的义务和无条件的禁止
混在一起、`uncovered` 条目没写理由。交付的那一份是 `SpecBundle`：约束，加上**表达不了
的条款和它们的理由**。`uncovered` 空着会警告——一份真实 SOP 很少一条都落不到内置类型
之外，而让模型来写声明时，它从不会主动说「这条我表达不了」。

`TEMPLATE.zh.md` 是填表工作单：只覆盖人不可替代的那几件事——写留出用例、拆混写的条款、
判定表达不了、调措辞——每一步都写明机器会替你查什么。
