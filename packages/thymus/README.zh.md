...
线上流量是留出用例最好的来源——用户真的那么说了，而 SOP 原文没列过。
`splitBadCases()` / `applyBadCases()` 把人工标注过的样本并进评测集，
并由三条机械纪律护着，每一条都是被实测逼出来的：

- **「算不算违规」必须人判。** 线上没有标准答案，让模型标就回到自己给自己打分——
  那个落差量过：自评几乎全绿，换成外部题只有 8/13。
- **分流确定性**（按内容哈希），没法反复重抽直到留出侧只剩容易的。
- **留出侧只报数字不报原文**（`redactHeldout()`）。看得到就会照着改，
  那测的是照差异改，不是泛化。

`compareReports()` 与上一版比，任一组掉了都算回归，留出集也算——措辞是未受控变量，
为一条新样本改一句话很容易把旧的弄坏。
